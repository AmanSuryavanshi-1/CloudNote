...
const Notes = (props) => {
  ...
  const [note, setNote] = useState({
    id: " ",
    etitle: " ",               // edited title
    edescription: " ",         // edited Description
    etag: "default",
  });
    ...
    <>
      ...
      <div className="row my-3">
        ...
        <div className="container"> 
         {notes.length===0 && 'No notes to display'}   {/* if notes.length===0 then it will return no  notes to display */}
         </div>
        {notes.map((note) => {
          return (
            <Noteitem key={note._id} updateNote={updateNote} showAlert={props.showAlert } note={note} />
          ); //Getting notes title from NoteState Using context
        })}
      </div>
    </>
  );
